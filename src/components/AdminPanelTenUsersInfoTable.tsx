...
const TenUsersInfoTable = ({
  data,
  users,
}: {
  data: UserAgent[];
  users: { email: string; user_agent_id: string[] }[];
}) => {
            ...
            return (
              <tr key={userAgent._id}>
                <td className="px-6 py-4 whitespace-nowrap">
                  {userIdToEmail[userAgent._id] || userAgent._id}
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                  {userAgent.visits.length}
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                  {mostFrequentValue(userAgent.visits, "entry_point")}
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                  {mostFrequentValue(
                    userAgent.visits.map((e) => e.os),
                    "name"
                  )}
                </td>
              </tr>
